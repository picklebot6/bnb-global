...
function showOnly(id) {
  ...
  viewVersion++;
  for (const view of ['loginView', 'homeView', 'appView', 'allRunsView', 'psoView', 'runView', 'startedView']) {
...
async function renderRoute() {
  ...
  if (params.get('runs') === 'all') return showAllRuns();
...
$('viewAllRuns').addEventListener('click', () => navigate(`/?workflow=${selectedWorkflow}&runs=all`));
$('allRunsBack').addEventListener('click', showAppView);
$('allRunsPeriod').addEventListener('change', renderAllRuns);
$('allRunsRefresh').addEventListener('click', () => showAllRuns(false));
$('activityPeriod').addEventListener('change', () => renderActivityChart(homeActivityRuns));
...
async function refreshHomeActivity() {
  ...
  try {
    ...
    if (version !== viewVersion) return;
    homeActivityRuns = results.flatMap(({ id, runs }) =>
      runs.map(run => ({ ...run, workflowId: id })),
    );
    renderActivityChart(homeActivityRuns);
  } catch (error) {
    if (version !== viewVersion) return;
...
function renderRuns(results) {
  ...
  $('runs').innerHTML = renderRunRows(recent);
        ...
        <div class="run-row run-${rowTone}">
          <div class="run-details">
            <div>
              ...
              ${run.status === 'in_progress' ? '<div class="run-cancel-hint">View log to cancel this run.</div>' : ''}
            </div>
          </div>
...
function renderAllRuns() {
  const period = $('allRunsPeriod').value;
  ...
  const visibleRuns = allRunsForWorkflow.filter(
    run => new Date(run.createdAt) >= start,
  );
  $('allRuns').innerHTML = visibleRuns.length
    ? renderRunRows(visibleRuns)
    : '<div class="run-meta">No workflow runs found for this period.</div>';
}

async function showAllRuns(resetPeriod = true) {
  showOnly('allRunsView');
  const workflow = workflows[selectedWorkflow];
  $('allRunsTitle').textContent = `${workflow.label} · All Runs`;
  if (resetPeriod) $('allRunsPeriod').value = 'today';
  $('allRuns').textContent = 'Loading runs…';
  $('allRunsRefresh').disabled = true;
  const version = viewVersion;
  try {
    const { runs } = await loadRuns(selectedWorkflow);
    if (version !== viewVersion) return;
    allRunsForWorkflow = runs
      .map(run => ({ ...run, workflowId: selectedWorkflow }))
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    renderAllRuns();
  } catch (error) {
    if (version !== viewVersion) return;
    $('allRuns').textContent = error.message;
  } finally {
    if (version === viewVersion) $('allRunsRefresh').disabled = false;
  }
}

/** Renders the selected dashboard view for recent workflow activity. */
function renderActivityChart(runs) {
  if ($('activityPeriod').value === 'today') {
    renderTodayActivityChart(runs);
    return;
  }

  $('activitySubtitle').textContent = 'Runs started in the past 7 days';
  $('activityLegend').classList.remove('hidden');
  $('activityChart').classList.remove('automation-breakdown');
  $('activityChart').setAttribute('aria-label', 'Automation activity for the past 7 days');
  const statusTones = {
    success: 'success',
    failure: 'failure',
    timed_out: 'failure',
  ...
  const days = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      date,
      key: dayKey(date),
      counts: { success: 0, failure: 0, progress: 0, queued: 0 },
    };
  });
  ...
  for (const run of runs) {
    ...
    const status = run.status === 'completed'
      ? run.conclusion || 'completed'
      : run.status;
    const tone = statusTones[status] || 'queued';
    day.counts[tone]++;
  }

  const totals = days.map(day => Object.values(day.counts).reduce(
    (sum, count) => sum + count,
    0,
  ));
  ...
  $('activityChart').innerHTML = days.map((day, index) => {
    ...
    const bar = total === 0
      ...
      : `<div class="activity-bar" style="height:${Math.max(10, total / max * 100)}%">${segments}</div>`;

    return `<div class="activity-day" title="${dateLabel}: ${details}">
      <div class="activity-count">${total || ''}</div>
      <div class="activity-bar-slot">${bar}</div>
      <div class="activity-label">${day.date.toLocaleDateString([], { weekday: 'short' })}</div>
      <div class="activity-date">${day.date.toLocaleDateString([], { month: 'numeric', day: 'numeric' })}</div>
    </div>`;
  }).join('');
}

/** Renders today's run totals as one bar for each automation. */
function renderTodayActivityChart(runs) {
  const start = new Date();
  start.setHours(0, 0, 0, 0);
  const statusTones = {
    success: 'success',
  ...
  const totals = Object.entries(workflows).map(([id, workflow]) => ({
    id,
    label: workflow.label,
    counts: { success: 0, failure: 0, progress: 0, queued: 0 },
  })).map(item => {
    for (const run of runs) {
      if (run.workflowId !== item.id || new Date(run.createdAt) < start) continue;
      const status = run.status === 'completed'
        ? run.conclusion || 'completed'
        : run.status;
      item.counts[statusTones[status] || 'queued']++;
    }
    return {
      ...item,
      count: Object.values(item.counts).reduce((sum, count) => sum + count, 0),
    };
  });
  const max = Math.max(1, ...totals.map(item => item.count));

  $('activitySubtitle').textContent = 'Runs started today by automation';
  $('activityLegend').classList.remove('hidden');
  $('activityChart').classList.add('automation-breakdown');
  $('activityChart').setAttribute('aria-label', 'Today\'s runs by automation');
  $('activityChart').innerHTML = totals.map(item => {
    const bar = item.count === 0
      ? '<div class="activity-bar empty"></div>'
      : `<div class="activity-bar" style="height:${Math.max(10, item.count / max * 100)}%">${Object.entries(item.counts)
        .filter(([, count]) => count > 0)
        .map(([status, count]) => `<span class="activity-segment activity-${status}" style="flex:${count}"></span>`)
        .join('')}</div>`;
    return `<div class="activity-day" title="${item.label}: ${item.count} ${item.count === 1 ? 'run' : 'runs'}">
