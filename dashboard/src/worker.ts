...
async function createSession(secret: string): Promise<string> {
  const payload = `${Math.floor(Date.now() / 1000) + 60 * 60 * 12}`;
...
async function isAuthenticated(request: Request, secret: string): Promise<boolean> {
  const token = getCookie(request, 'bnb_session');
  ...
  try {
    const expiresAt = Number(new TextDecoder().decode(fromBase64Url(encoded)));
...
async function dispatchWorkflow(workflowFile: string, env: Env): Promise<Response> {
  ...
  if (!response.ok) {
    ...
    return json({ ok: false, error: body || `GitHub returned ${response.status}` }, response.status);
  }

  return json({ ok: true });
}

async function getLatestRuns(workflowFile: string, env: Env): Promise<Response> {
  const url = `https://api.github.com/repos/${OWNER}/${REPO}/actions/workflows/${encodeURIComponent(workflowFile)}/runs?branch=${encodeURIComponent(REF)}&per_page=5`;
  const response = await fetch(url, {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/login' && request.method === 'POST') {
      ...
      return json(
        ...
        200,
        {
          'Set-Cookie': `bnb_session=${encodeURIComponent(session)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=43200`,
        },
      );
    }
    ...
    if (url.pathname === '/api/logout' && request.method === 'POST') {
      return json(
        { ok: true },
        200,
        { 'Set-Cookie': 'bnb_session=; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=0' },
      );
    }
    ...
    if (url.pathname === '/api/workflows' && request.method === 'GET') {
      ...
      return json({ ok: true, workflows: Object.entries(WORKFLOWS).map(([id, value]) => ({ id, ...value })) });
    }

    const dispatchMatch = url.pathname.match(/^\/api\/workflows\/([^/]+)\/dispatch$/);
    if (dispatchMatch && request.method === 'POST') {
      if (!(await isAuthenticated(request, env.SESSION_SECRET))) return json({ ok: false, error: 'Unauthorized' }, 401);
      const workflow = WORKFLOWS[dispatchMatch[1]];
      if (!workflow) return json({ ok: false, error: 'Unknown workflow.' }, 404);
      return dispatchWorkflow(workflow.file, env);
    }

    const runsMatch = url.pathname.match(/^\/api\/workflows\/([^/]+)\/runs$/);
    if (runsMatch && request.method === 'GET') {
      if (!(await isAuthenticated(request, env.SESSION_SECRET))) return json({ ok: false, error: 'Unauthorized' }, 401);
      const workflow = WORKFLOWS[runsMatch[1]];
      if (!workflow) return json({ ok: false, error: 'Unknown workflow.' }, 404);
      return getLatestRuns(workflow.file, env);
    }
